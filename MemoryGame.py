...
def setup(level):
    ...
    number_count = min(number_count, 20)  # 최대 숫자수 20개로 처리
...
def shuffle_grid(number_count):
    ...
    grid = [[0 for col in range(columns)] for row in range(rows)]    # [0,0,0,0,0,0,0,0,0] * 5

    number = 1 # 시작 숫자를 1부터 number_count 까지, 만약 5라면 5까지 숫자를 랜덤으로 배치한다
    while number <= number_count:
        row_idx = randrange(0, rows)  # 0, 1, 2, 3, 4 중에서 랜덤 뽑기
        col_idx = randrange(0, columns) # 0,1,2,3,4,5,6,7,8 중에서 랜덤 뽑기
...
def display_game_screen():
    global hidden
    if not hidden:
        elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000  # 초단위로 바꿈
...
def check_buttons(pos):
    global start, start_ticks  # 밖에서 선언된 start 변수임을 명시해주는것
...
def check_number_buttons(pos):
    ...
    for button in number_buttons:
        if button.collidepoint(pos):
            if button == number_buttons[0]:   # 올바른 숫자 클릭 한것
                #print("correct")
                ...
                game_over()
            break
    if len(number_buttons) == 0:  # 모든 숫자를 다 맞혔다면? 레벨을 높여서 다시 시작하면으로 간다
        start = False
        hidden = False
        curr_level += 1
        setup(curr_level)
...
number_buttons = [] # 플레이어가 눌러야 하는 버튼들을 관리하는 리스트

curr_level = 1  # 현재 레벨 1

display_time = None # 숫자를 보여주는 시간
start_ticks = None # 시간 계산  (현재시간 정보 저장)
...
pygame.time.delay(3000) # 5초정도 자기 레벨 확인 후 종료
# 게임 종료
pygame.quit()
